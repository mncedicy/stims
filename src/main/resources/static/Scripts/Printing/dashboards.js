$(document).ready(function () {
    ...
    var chartOfficer;
    function loadChartOfficer(series1,days) {
        var options = {
            series: series1,
            chart: {
                type: 'bar',
    ...
    function loadChartCharge(series, charges, chargesShort) {
        ...
        if (!chartCharge) {
            chartCharge = new ApexCharts(document.querySelector("#chartCharge"), options);
            chartCharge.render();
        }
        else {
    ...
    function printNoticeReport() {
        ...
        showLoader();
        fetch(httpsapi + "/report/printNoticeReport?grouping=Charge",
            {
                method: 'POST',
                headers: {
